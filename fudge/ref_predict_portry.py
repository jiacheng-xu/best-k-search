...
logging.basicConfig(
    encoding="utf-8",
    level=logging.DEBUG,
    handlers=[logging.FileHandler("fudge_original.log"), logging.StreamHandler()],
)

def visualize_logits(logits, raw_indicies, name, tokenizer, topk=5):
    # assume logits in batch
    tmp_logit = logits[0]
    values, indices = torch.topk(tmp_logit, k=topk, dim=-1)  # batch x pretopk
    tmp_raw = raw_indicies[0]
    out = []
    for i in range(topk):
        out.append(tokenizer.decode(tmp_raw[indices[i]]))
    logging.info(name+":\t\t"+" ".join(out))
...
def predict_couplet(gpt_model, gpt_tokenizer, iambic_model, rhyme_model, newline_model, input_text, dataset_info, rhyme_info, precondition_topk, postcondition_topk, condition_lambda=1.0, device='cuda'):
    ...
    line = predict_iambic_pentameter_line(gpt_model, 
                        ...
                        device=device)
    logging.info(current_text, line)
...
def predict_iambic_pentameter_line(gpt_model, gpt_tokenizer, iambic_model, rhyme_model, newline_model, current_text, current_line_text, rhyme_group, dataset_info, rhyme_info, precondition_topk, postcondition_topk, banned_tokens=POETRY_BANNED_TOKENS, condition_lambda=1.0, device='cuda', length_cutoff=30):
    ...
    with torch.no_grad():
        ...
        lengths = torch.LongTensor([encoded_input.shape[1]]).to(device)  #TODO
        ...
        for _ in range(length_cutoff): # really shouldn't have a line this long anyway
            ...
            logging.info(f"Input: {gpt_tokenizer.decode(encoded_input[0])}\tid: {encoded_input[0]}")
            ...
            logging.info([gpt_tokenizer.decode(x) for x in top_indices[0][:5]])
            ...
            visualize_logits(iambic_logits,top_indices,"iambic",gpt_tokenizer,5)
            visualize_logits(rhyme_logits,top_indices,"rhyme",gpt_tokenizer,5)
            visualize_logits(newline_logits,top_indices,"newli",gpt_tokenizer,5)
            ...
            visualize_logits(full_logits,top_indices,"full",gpt_tokenizer,5)
            ...
            logging.info("FINAL: " +gpt_tokenizer.decode(next_indices[0]))
...
if __name__=='__main__':
    ...
    args.input_text = "Pity the world, or else this glutton be,\n"
