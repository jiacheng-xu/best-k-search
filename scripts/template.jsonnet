...
// local task_name="enfr";
// local task_name="ende";
...
local beam_size = if (task_name=="enfr" || task_name=="ende") then 10 else 10; 
// 10 for all
...
local _algos = [bs, dbs, dbs_full ] + bfs + sample["typical_p"] + sample["top_p"] + sample1["typical_p"] + sample1["top_p"];


local _algos = sample["typical_p"] + sample["top_p"] + sample1["typical_p"] + sample1["top_p"];
...
local TaskConfig(task) = {
  local config= if task=="squad" then config_squad else 
        ...
        if task == "enfr" then config_opus else
        if task == "ende" then config_opus else
        config_quoref,
  "config":config
};
